...
## Shared transactional email delivery

- `email_delivery.py` provides deployment-configured SMTP, AWS SES v2, Aliyun DirectMail, and Resend adapters behind `send_email`. `mfa.py` retains message composition, SQL challenge state, and send limits. Additional callers must enforce their own authorization and send budgets.
- Blank `EMAIL_PROVIDER` preserves legacy `SMTP_HOST` / `SMTP_FROM` deployments; explicit `none` disables delivery. Credentials remain outside SQL, Redis, and settings responses. New adapters must update `.env.example`, Compose forwarding, and `docs/email-delivery.md` together.
- Provider acceptance is not proof of inbox delivery. Do not retry or fail over automatically after an ambiguous failure. Keep provider exceptions and response bodies out of MFA responses and logs; preserve recovery-code access through outages.
- AWS SDK clients expose `close()` but are not context managers; use `contextlib.closing`. Offline SES tests must exercise the real SDK with Stubber to catch lifecycle and request-shape mistakes. Aliyun signing has a fixed published test vector. Give oversized parameterized test payloads short explicit IDs to avoid Windows environment-size errors in `PYTEST_CURRENT_TEST`.
